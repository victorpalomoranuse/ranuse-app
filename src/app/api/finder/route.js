...
// Dominios permitidos (transfermarkt bloquea a Claude, por eso no lo incluimos)
...
TIENES UNA HERRAMIENTA web_search RESTRINGIDA A FUENTES FIABLES:
- besoccer.com (principal para plantillas y estadísticas)
- marca.com, as.com (para información y trayectoria)
- wikipedia.org (para perfiles biográficos, edades, clubes)
- fotmob.com (estadísticas complementarias)
...
2. Busca plantillas reales, edades reales, clubes actuales y antigüedad en el club.
...
Haz búsquedas en BeSoccer, Marca, AS o Wikipedia para verificar plantillas, edades y antigüedad en el club antes de responder. Responde SOLO con el JSON pedido.`;
    ...
    if (!textoFinal) {
      return Response.json(
        { error: "No se obtuvo texto final del modelo." },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      return Response.json(
        { error: "La respuesta no era JSON válido", raw: textoFinal.slice(0, 800) },
        { status: 500 }
      );
    }
