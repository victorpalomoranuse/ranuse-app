        ...
        <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
                          ...
                          <div style={{ padding: "8px 12px 12px", borderTop: "1px solid rgba(255,255,255,0.07)" }}>
                            ...
                            {mensajesProspecto.length > 0 && (
                              <div style={{ marginTop: 10 }}>
                                <div style={{ color: "#d4a853", fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, marginBottom: 6 }}>📨 Historial ({mensajesProspecto.length})</div>
                                {mensajesProspecto.map(m => (
                                  <div key={m.id} style={{ background: "rgba(0,0,0,0.3)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 6, padding: "6px 8px", marginBottom: 5, fontSize: 11 }}>
                                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                                      <span style={{ color: "#d4a853", fontWeight: 700 }}>#{m.secuencia} · {m.tipo_mensaje || "sin plantilla"}</span>
          ...
          {tab === "buscador" && <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: 14, marginBottom: 16 }}>
              ...
              <button onClick={buscar} disabled={finderLoading} style={{ width: "100%", background: finderLoading ? "#1a1a1a" : "linear-gradient(135deg, #d4a853, #c49240)", border: "none", borderRadius: 10, padding: 12, color: finderLoading ? "#444" : "#000", fontWeight: 700, fontSize: 13, cursor: finderLoading ? "default" : "pointer" }}>{finderLoading ? "Buscando..." : "🔍 Buscar prospectos nuevos"}</button>
            </div>
          ...
          {tab === "plantillas" && <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(212,168,83,0.3)", borderRadius: 10, padding: 12, marginBottom: 16 }}>
              <div style={{ color: "#d4a853", fontSize: 10, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 8 }}>➕ Nueva plantilla</div>
              ...
              <button onClick={async () => { if (nuevaPlantilla.nombre && nuevaPlantilla.estructura) { await guardarPlantilla(nuevaPlantilla); setNuevaPlantilla({ nombre: "", estructura: "", descripcion: "" }); } }} style={{ width: "100%", background: "linear-gradient(135deg, #d4a853, #c49240)", border: "none", borderRadius: 8, padding: 9, color: "#000", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>Guardar plantilla</button>
            </div>
            ...
            {plantillas.map(pl => (
              <div key={pl.id} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: 12, marginBottom: 8 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                  <strong style={{ color: "#d4a853", fontSize: 13, textTransform: "uppercase" }}>{pl.nombre}</strong>
          ...
          {tab === "objetivos" && <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
            ...
            {objetivos.filter(o => o.tipo === "mensual").map(o => (
              <div key={o.id} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: 10, marginBottom: 6, display: "flex", alignItems: "center", gap: 8 }}>
                ...
                </> : <div style={{ display: "flex", gap: 12, fontSize: 11, color: "#aaa" }}><span>P: <b style={{ color: "#fff" }}>{o.prospeccion}</b></span><span>I: <b style={{ color: "#fff" }}>{o.interesados}</b></span><span>V: <b style={{ color: "#fff" }}>{o.ventas}</b></span><span>€: <b style={{ color: "#d4a853" }}>{Number(o.facturacion).toLocaleString("es-ES")}</b></span></div>}
              </div>
            ))}
          </div>}
            ...
            {analisis.map((a, i) => (
              <div key={i} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: 13, marginBottom: 9 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 7 }}>
                  <strong style={{ fontSize: 13, textTransform: "uppercase", color: "#d4a853" }}>{a.nombre}</strong>
                  <span style={{ color: a.tasa_respuesta >= 20 ? "#4ade80" : a.tasa_respuesta >= 10 ? "#d4a853" : "#f87171", fontFamily: "monospace", fontSize: 14, fontWeight: 700 }}>{a.tasa_respuesta}%</span>
                </div>
                <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 4, height: 5, marginBottom: 8 }}>
                  <div style={{ background: a.tasa_respuesta >= 20 ? "#4ade80" : a.tasa_respuesta >= 10 ? "#d4a853" : "#f87171", width: `${Math.min(a.tasa_respuesta, 100)}%`, height: "100%", borderRadius: 4, opacity: 0.8 }} />
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, fontSize: 11 }}>
                  ...
                  <div><div style={{ color: "#666" }}>Ventas</div><div style={{ fontFamily: "monospace", color: "#d4a853", fontSize: 14 }}>{a.ventas}</div></div>
                </div>
              </div>
            ))}
          </div>}
...
function BarraObjetivo({ titulo, actual, objetivo, editar, onChange, formato }) {
  ...
  return <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: 14, marginBottom: 10 }}>
    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
      ...
      <div style={{ fontSize: 11, color: "#aaa" }}><span style={{ color: "#d4a853", fontWeight: 700 }}>{fmt(actual)}</span> / {editar ? <input type="number" defaultValue={objetivo} onBlur={e => onChange(Number(e.target.value) || 0)} style={{ ...inputStyle, display: "inline-block", width: 80, padding: "2px 6px", fontSize: 11 }} /> : fmt(objetivo)}</div>
    </div>
    <div style={{ background: "rgba(255,255,255,0.05)", borderRadius: 4, height: 6 }}><div style={{ background: pct >= 100 ? "#4ade80" : "#d4a853", width: `${pct}%`, height: "100%", borderRadius: 4 }} /></div>
