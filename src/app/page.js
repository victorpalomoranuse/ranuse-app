...
const ESTADOS = {
  sin_contactar: { label: "Sin contactar", color: "#666" },
  no_contesta:   { label: "No contesta",   color: "#555" },
  leido:         { label: "Leído",         color: "#f59e0b" },
  activo:        { label: "Activo",        color: "#4ade80" },
  negociacion:   { label: "Negociación",   color: "#d4a853" },
...
export default function App() {
  ...
  const quickActions = ["¿Quién toca hoy?", "DM para portero Hypermotion", "Resumen de abril", "Seguimientos urgentes"];
  ...
  for (const p of prospectosFiltrados) {
    const k = p.estado || "sin_contactar";
    if (!grupos[k]) grupos[k] = [];
    grupos[k].push(p);
  }
  const ordenEstados = ["negociacion", "activo", "leido", "sin_contactar", "no_contesta", "ganado", "rechazado"];
        ...
        <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
          ...
          {tab === "prospectos" && <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
              <Pill label="Todos" active={filtroEstado==="all"} onClick={() => setFiltroEstado("all")} />
              {Object.entries(ESTADOS).map(([k, v]) => (
                <Pill key={k} label={v.label} active={filtroEstado===k} onClick={() => setFiltroEstado(k)} />
              ))}
            </div>
            ...
            {!loading && ordenEstados.map(estado => {
              const grupo = grupos[estado];
              if (!grupo?.length) return null;
              const cfg = ESTADOS[estado];
              return (
                <div key={estado} style={{ marginBottom: 18 }}>
                  <div style={{ color: cfg.color, fontSize: 10, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 8 }}>
                  ...
                  {grupo.map(p => (
                    <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 12px", borderRadius: 9, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", marginBottom: 6 }}>
                      ...
                      <select value={p.estado} onChange={e => actualizarEstado(p.id, e.target.value)} style={{ background: "rgba(0,0,0,0.4)", color: cfg.color, border: `1px solid ${cfg.color}55`, borderRadius: 6, fontSize: 10, padding: "3px 5px" }}>
          ...
          {tab === "buscador" && <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
            <p style={{ color: "#666", fontSize: 12, marginBottom: 14, lineHeight: 1.5 }}>La IA busca jugadores reales <strong style={{ color: "#d4a853" }}>que no están en tu base de datos</strong>.</p>
