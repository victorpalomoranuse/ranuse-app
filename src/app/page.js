      ...
      <div className="ranuse-container" style={{ maxWidth: 520, margin: "0 auto", display: "flex", flexDirection: "column", height: "100dvh" }}>
        <div style={{ padding: "14px 18px 0", borderBottom: "1px solid rgba(255,255,255,0.07)", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", overflowX: "auto", gap: 0 }}>
            {[["chat", "Chat"], ["prospectos", "Prospectos"], ["buscador", "🔍 Buscador"], ["metricas", "Métricas"]].map(([id, label]) => (
        ...
        <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
          ...
          {tab === "metricas" && <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
            ...
            {metricas.map(m => {
              const tasaLeido = m.contactados > 0 ? ((m.leidos / m.contactados) * 100).toFixed(1) : "0";
              const tasaResp = m.contactados > 0 ? ((m.responden / m.contactados) * 100).toFixed(1) : "0";
              const tasaVenta = m.contactados > 0 ? ((m.ventas / m.contactados) * 100).toFixed(1) : "0";
              return (
                <div key={m.mes} style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: 14, marginBottom: 10 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                    ...
                    <span style={{ color: "#d4a853", fontFamily: "monospace", fontSize: 11 }}>{tasaResp}% resp.</span>
                  </div>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10, fontSize: 11 }}>
                    <div><div style={{ color: "#666" }}>Contactados</div><div style={{ fontFamily: "monospace", fontSize: 18, color: "#fff" }}>{m.contactados}</div></div>
                    <div><div style={{ color: "#666" }}>Leídos ({tasaLeido}%)</div><div style={{ fontFamily: "monospace", fontSize: 18, color: "#f59e0b" }}>{m.leidos}</div></div>
                    <div><div style={{ color: "#666" }}>Respuestas</div><div style={{ fontFamily: "monospace", fontSize: 18, color: "#4ade80" }}>{m.responden}</div></div>
                    <div><div style={{ color: "#666" }}>Interesados</div><div style={{ fontFamily: "monospace", fontSize: 18, color: "#22c55e" }}>{m.interesados}</div></div>
                    <div><div style={{ color: "#666" }}>Rechazos</div><div style={{ fontFamily: "monospace", fontSize: 18, color: "#f87171" }}>{m.rechazados}</div></div>
                    <div><div style={{ color: "#666" }}>💰 Ventas ({tasaVenta}%)</div><div style={{ fontFamily: "monospace", fontSize: 18, color: "#d4a853" }}>{m.ventas}</div></div>
                  </div>
                </div>
              );
            })}
            ...
            <div style={{ color: "#666", fontSize: 11, textAlign: "center", padding: 16, marginTop: 10, border: "1px dashed rgba(255,255,255,0.1)", borderRadius: 10 }}>
              🚧 El embudo completo (vídeo, llamadas, €) y los objetivos llegan en la FASE B.
            </div>
          </div>}
