...
export default function FinanzasTab({
  ...
  // Números simples: lo que entra, lo que sale, lo que queda
  ...
  const periodoLabel = filtroAno === "all"
    ? "Acumulado total"
    : filtroQ === "all"
    ? `Año ${filtroAno}`
    : `${TRIMESTRES[filtroQ].label} ${filtroAno}`;
  ...
  if (!finanzasUnlocked) return (
    <div style={{ flex: 1, overflowY: "auto", padding: 16 }}>
      <div style={{ maxWidth: 320, margin: "40px auto", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(190,176,162,0.2)", borderRadius: 12, padding: 20 }}>
        ...
        <button onClick={unlockFinanzas} style={{ width: "100%", background: "linear-gradient(135deg, #beb0a2, #a89686)", border: "none", borderRadius: 8, padding: 9, color: "#000", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
          Entrar
        </button>
      </div>
    </div>
      ...
      {vista === "movimientos" && <>
        ...
        <div style={{ marginBottom: 14 }}>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 6 }}>
            ...
            {anos.map(a => (
              <FiltroBtn key={a} label={a} active={filtroAno === a} onClick={() => { setFiltroAno(a); setFiltroQ("all"); }} />
            ))}
          </div>
          {filtroAno !== "all" && (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
          ...
          <Tarjeta
            label="Saldo"
            valor={resumen.saldo}
            color={resumen.saldo >= 0 ? "#beb0a2" : "#f87171"}
            grande
          />
        </div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
          ...
          <button
            onClick={() => setVerCuadres(!verCuadres)}
            style={{ background: verCuadres ? "rgba(255,255,255,0.08)" : "rgba(190,176,162,0.1)", border: "1px solid rgba(190,176,162,0.25)", borderRadius: 8, padding: "9px 12px", color: "#beb0a2", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>
            ⚖️
          </button>
          <button
            onClick={() => setVerCategorias(!verCategorias)}
            style={{ background: "rgba(190,176,162,0.1)", border: "1px solid rgba(190,176,162,0.25)", borderRadius: 8, padding: "9px 12px", color: "#beb0a2", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>
            🏷️
          </button>
        </div>
        ...
        {verCuadres && (
          <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 12, padding: 12, marginBottom: 14 }}>
            ...
            <div style={{ borderTop: "1px solid rgba(255,255,255,0.07)", paddingTop: 10 }}>
              ...
              <button
                onClick={async () => { await crearCuadre(nuevoCuadre); setNuevoCuadre(c => ({ ...c, saldo_real: "", nota: "" })); }}
        ...
        {/* FORMULARIO NUEVO MOVIMIENTO — simplificado */}
        {crearMov && (
          <div style={{ background: "rgba(190,176,162,0.04)", border: "1px solid rgba(190,176,162,0.25)", borderRadius: 10, padding: 12, marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", gap: 5, marginBottom: 10 }}>
              {[["ingreso","+ Ingreso","rgba(34,197,94,0.18)","#4ade80"],["gasto","− Gasto","rgba(248,113,113,0.18)","#f87171"]].map(([t, label, bg, col]) => (
                <button key={t}
                  onClick={() => setNuevoMov(m => ({ ...m, tipo: t, categoria: "" }))}
            ...
            <input
              ...
              onChange={e => setNuevoMov(m => ({ ...m, fecha: e.target.value }))}
              style={{ ...inputStyle, marginBottom: 6 }}
            />
            <input
              type="number"
              placeholder="Importe (€)"
              value={nuevoMov.importe}
              onChange={e => setNuevoMov(m => ({ ...m, importe: e.target.value }))}
              style={{ ...inputStyle, marginBottom: 6 }}
            />
            <select
              value={nuevoMov.categoria || ""}
              onChange={e => setNuevoMov(m => ({ ...m, categoria: e.target.value }))}
              style={{ ...inputStyle, marginBottom: 6 }}
            >
            ...
            <select
              value={nuevoMov.cuenta || "banco"}
              onChange={e => setNuevoMov(m => ({ ...m, cuenta: e.target.value }))}
              style={{ ...inputStyle, marginBottom: 6 }}
            >
              <option value="banco">Banco</option>
              <option value="caja">Caja</option>
            ...
            <input
              ...
              value={nuevoMov.proyecto || ""}
              onChange={e => setNuevoMov(m => ({ ...m, proyecto: e.target.value }))}
              style={{ ...inputStyle, marginBottom: 6 }}
            />
            <input
            ...
            <button
              ...
              disabled={!nuevoMov.importe || !nuevoMov.fecha}
        ...
        {verCategorias && (
          <div style={{ background: "rgba(190,176,162,0.04)", border: "1px solid rgba(190,176,162,0.2)", borderRadius: 10, padding: 12, marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", gap: 5, marginBottom: 6 }}>
              ...
              <button
                onClick={crearCategoriaFin}
                disabled={!nuevaCategoria.nombre.trim()}
                style={{ background: nuevaCategoria.nombre.trim() ? "#beb0a2" : "#1a1a1a", color: nuevaCategoria.nombre.trim() ? "#000" : "#444", border: "none", borderRadius: 6, padding: "0 12px", fontSize: 14, fontWeight: 700, cursor: nuevaCategoria.nombre.trim() ? "pointer" : "default" }}>
                +
              </button>
            </div>
            {categoriasFin.map(c => (
            ...
            {porProyecto.map(p => {
              ...
              return (
                <div key={p.nombre} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                  <div
                    onClick={() => setProyectoAbierto(abierto ? null : p.nombre)}
              ...
              <div style={{ padding: "9px 12px", display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <button
                  onClick={() => {
                    ...
                    else { setEditandoMov(m.id); setEditCampos({ fecha: m.fecha, importe: m.importe, categoria: m.categoria || "", cuenta: m.cuenta || "banco", proyecto: m.proyecto || "", descripcion: m.descripcion || "" }); }
                  }}
                  style={{ background: "none", border: "none", color: isEditing ? "#beb0a2" : "#444", cursor: "pointer", fontSize: 12, flexShrink: 0 }}>
                  ✎
                </button>
                <button
                  onClick={() => borrarMovimiento(m.id)}
                  style={{ background: "none", border: "none", color: "#444", cursor: "pointer", fontSize: 12, flexShrink: 0 }}>
                  ✕
                </button>
              </div>
